...
void CreateChangeVIPMenu(int iClient, const char[] newGroup, const char[] time)
{
...
public int Selector(Menu menu, MenuAction action, int client, int item)
{
    switch(action)
    {
		...
		case MenuAction_Cancel:
		{
			...
			{
				char sTime[32], group[256];
				menu.GetItem(1, sTime, sizeof(sTime), _, group, sizeof(group));
				CreateChangeVIPMenu(client, group, sTime);
			}
		}
